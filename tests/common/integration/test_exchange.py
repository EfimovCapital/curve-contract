...
def test_simulated_exchange(
    ...
    while st_coin:
        ...
        i, j = [int(st_coin.pop() * n_coins) for _ in range(2)]
        if i == j:
            # if send and recv are the same, adjust send
            i = abs(i - 1)

        value = 5 * rate_mul[i] // st_divisor.pop()

        x_0 = underlying_coins[i].balanceOf(bob)
        y_0 = underlying_coins[j].balanceOf(bob)
        underlying_coins[i].approve(swap, 0, {'from': bob})
        underlying_coins[i].approve(swap, value, {'from': bob})

        amount = int(0.5 * value * rate_mul[j] / rate_mul[i])
        swap.exchange_underlying(i, j, value, amount, {'from': bob})

        x_1 = underlying_coins[i].balanceOf(bob)
        y_1 = underlying_coins[j].balanceOf(bob)

        dy_m = curve_model.exchange(i, j, value * max(rate_mul) // rate_mul[i])
        dy_m = dy_m * rate_mul[j] // max(rate_mul)

        assert x_0 - x_1 == value
        assert (y_1 - y_0) - dy_m <= dy_m * 1e-10
        assert swap.get_virtual_price() > old_virtual_price
        assert wrapped_coins[i].balanceOf(swap) >= swap.balances(i)
        assert wrapped_coins[j].balanceOf(swap) >= swap.balances(j)
    ...
    assert sum(final_balances[i] * rates[i] / 1e18 for i in range(n_coins)) > 300 * max(rate_mul)
